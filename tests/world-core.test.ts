...
test('nine fixed visual validation views are complete and terrain-safe', () => {
  const expected = ['castle-hero', 'courtyard-stair', 'village-approach', 'forest-edge', 'lake-shore', 'aerial-orbit', 'great-hall', 'station-hall', 'library-hall'];
  for (let seedIndex = 1; seedIndex <= 20; seedIndex += 1) {
    ...
    for (const view of manifest.validationViews) {
      ...
      assert.ok(view.position[1] >= ground + (view.id === 'great-hall' || view.id === 'station-hall' || view.id === 'library-hall' ? 1.6 : 6.8));
    }
  }
});
