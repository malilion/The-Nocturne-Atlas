...
**[線上體驗](https://malilion.github.io/The-Nocturne-Atlas/)**
...
日夜按鍵會完整轉換天空、霧、星光、天體光源、水面高光、補光、曝光、Bloom 與選用的環境聲景。夜間使用月光主光源、無陰影冷色補光、提高的半球環境光、較大聲的環境音量與較淺的霧，讓城堡與地形輪廓更清楚，同時保留月夜氛圍。聲音預設靜音，必須由使用者主動開啟。
...
生成器 `1.9.0` 將原本實心的城堡大廳改為中空建築外殼，同時保留外部屋頂輪廓。場景 `8` 會呈現具有長桌、長椅、講台、旗幟、發光窗戶、屋樑、二十四根漂浮蠟燭，以及範圍受限的暖色燭光補光；相同種子會重現相同的蠟燭配置。十四扇塔樓窗戶現在共用單一 InstancedMesh 批次，在維持立面外觀的同時，讓完整 1080p 城堡視角低於原本的 Draw Call 上限。
...
生成器 `2.4.0` 加入原創的 Web Audio 環境聲景：透過 Web Audio 圖表循環播放的原創音樂〈Beyond the Mist〉，日夜切換只調整總音量、不會中斷播放。霧岔任務的三次操作會依進程產生逐步豐富的合成提示和弦。聲音只能透過頂部 `Sound` 或 `M` 主動開啟，會跟隨世界重建更新，並具備明確的清理生命週期。
...
目前版本包含完成度完整的原始垂直切片，以及地面碰撞導航、三個可進入室內場景、兩名具有動作的車站角色、第一條完整固定種子跨區域任務、原創環境聲景、鏡頭距離區域串流，以及橫跨八個區域的固定種子世界常駐角色群體。
